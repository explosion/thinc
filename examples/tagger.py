...
class WindowTagger(Network):
    width = 32
    n = 1

    def setup(self, nr_class):
        self.layers = [
            WindowEncode(vectors={}, W=None, nr_out=self.width),
            ReLu(nr_out=self.width),
            ReLu(nr_out=self.width),
            Softmax(nr_out=nr_class)
        ]
...
def main():
    ...
    model = WindowTagger(nr_class)
    for X, y in train_data:
        for x in X:
            model.layers[0].add_vector(x, model.width, add_gradient=True)
