...
os.environ["MYPYPATH"] = os.path.dirname(os.path.dirname(this_file_dir))
...
class DataDrivenTestCase(pytest.Item):  # type: ignore  # inheriting from Any
    ...
    output: Optional[List[str]] = None  # Output for the first pass
    output2: Optional[Dict[int, List[str]]] = (
        None
...
def has_stable_flags(testcase: DataDrivenTestCase) -> bool:
    assert testcase.input is not None
