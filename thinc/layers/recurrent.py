...
def init(
    ...
    nO = model.get_dim("nO")
    model.set_param("initial_cells", model.ops.alloc((nO,)))
    model.set_param("initial_hiddens", model.ops.alloc((nO,)))
...
def forward(model: Model[InT, OutT], Xp: InT, is_train: bool) -> Tuple[OutT, Callable]:
    ...
    Y = model.ops.alloc((X.shape[0], X.shape[1], nO))
    ...
    def backprop(dYp: OutT) -> InT:
        ...
        d_state = (
            step_model.ops.alloc((dY.shape[1], nO)),
            step_model.ops.alloc((dY.shape[1], nO)),
        )
        dX = step_model.ops.alloc((dY.shape[0], dY.shape[1], nI))
...
def _get_initial_state(model, n, nO):
    initial_cells = model.ops.alloc((n, nO))
    initial_hiddens = model.ops.alloc((n, nO))
